  ...
  try {
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      ...
      const namesOk = summaries.every(
        (item, index) =>
          item.theme === expectedThemes[index] &&
          isOfficialTheme(item.theme),
      );
      ...
      if (!pinned.length) {
        ...
        // Find the requested quotes first.
        // Array.find() can return undefined, so we validate the results
        // before treating them as QuoteIn objects.
        const foundQuotes = ids.map((id) =>
          quotesForInsert.find((quote) => quote.id === id),
        );

        if (foundQuotes.some((quote) => !quote)) {
          extra = "\nquote_ids must exist in the candidate list.";
          continue;
        }

        selectedQuotes = foundQuotes as QuoteIn[];
      }
      ...
      if (payload.mode === "validated_ai") {
        ...
        // Titles are re-applied by the client from the approved set if
        // provided as topThemes meta.
      }
      ...
      const assembled = {
        ...
        methodology: withMethodologyProvenance(
          methodologyBase,
          "ai",
        ),
      };
      ...
      if (
        containsPiiPattern(blob) ||
        containsPiiPattern(assembled.methodology)
      ) {
        extra =
      ...
      if (words > WORD_LIMIT) {
        extra = `\nThe assembled note is ${words} words. Reduce prose by ${
          words - WORD_LIMIT
        } words. Do not rewrite quotes.`;
        continue;
      }
